...
from users.models import AutoSuggestWords, LoginLog, SharedNote, Token
...
from complaint_management.settings import logger, RESET_PASSWORD_LINK, JWT_SECRET, TOKEN_EXPIRY, REFRESH_TOKEN_EXPIRY, \
...
class SharedNotesViewSet(ListCreateViewSet):
    permission_classes = ()
    ...
    def get_queryset(self):
        ...
        end_date = self.request.query_params.get("to")
        note_text = self.request.query_params.get("note_text")
        queryset = SharedNote.objects.filter(shared_with=user_id).order_by(
            "-created_at"
        )
        ...
        try:
            ...
            return queryset.none()
        if note_text:
            queryset = queryset.filter(user_note__note_text__icontains=note_text)
        ...
        queryset = queryset.annotate(
            note_text=F("user_note__note_text"),
            shared_by=F("user_note__user_id__email"),
        )
        return queryset

    def create(self, request, *args, **kwargs):
        user_id = request.user.get("user_id")
        shared_note = request.data.get("shared_note")
        ...
        try:
            shared_note = UserNote.objects.get(id=shared_note)
        except ObjectDoesNotExist:
            return Response(
                {"code": 204, "message": get_message(204)},
                status=status.HTTP_400_BAD_REQUEST,
        ...
        try:
            for shared_with_id in shared_with:
                try:
                    shared_with = get_user_model().objects.get(id=shared_with_id)
                    SharedNote.objects.create(
                        user_id=user_obj, shared_with=shared_with, user_note=shared_note
                    )
...
class AutoCompleteViewSet(ListViewSet):
    queryset = AutoSuggestWords.objects.all()
    serializer_class = EmptySerializer
    authentication_classes = [JwtTokensAuthentication]
    ...
    def list(self, request, *args, **kwargs):
        ...
        if search_value:
            queryset = queryset.filter(search_value__istartswith=search_value)
        queryset = queryset.values("search_value").distinct()
        search_values = list(queryset.values_list("search_value", flat=True))
        return Response(
            {"code": 200, "message": get_message(200), "results": search_values}
        )
